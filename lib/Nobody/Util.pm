...
BEGIN {
  no strict 'subs';
  add_use( Carp         => '( @Carp::EXPORT_OK, @Carp::EXPORT )' );
  add_use( FindBin  =>    'qw( $Bin $Dir $RealBin $Script $RealScript )' );
...
BEGIN {
#      croak "done" unless grep { /croak/ } @EXPORT;
#      eex(\%use);
  if ($] >= 5.036) {
...
BEGIN {
#      @EXPORT = List::Util::uniq(
#        splice(@EXPORT),
#      );
#      for( qw( strftime mktime path Bin RealBin Script RealScript ), qw( Bin RealBin Script RealScript  ) ){
#        my($qr)=qr($_);
#        eex( grep /$qr/, @EXPORT );
#      };
  @EXPORT_OK=@EXPORT;
